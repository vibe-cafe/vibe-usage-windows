...
//! can't have adjacent text, so we render the numbers INTO the icon bitmap
//! with an embedded 5×7 pixel font (white glyphs + 1px shadow, transparent
//! background) and put the full-precision values in the tray tooltip.
//! One line (cost or tokens) renders large; both toggles on → two stacked
//! lines (mirrors the macOS stacked layout).
